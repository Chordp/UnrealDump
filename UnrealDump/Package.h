...
using OffsetTup = tuple<string, string, string, size_t, size_t>;
class Package
{
...
public:
	...
	void Process(vector<UEObject> &packageObjects,std::unordered_map<UEObject, bool>& processedObjects);
	bool Save();
	bool SaveOffset(unordered_map<string, vector<string>>&j);
private:
	...
	void PrintClass(std::ostream& os, const Class& c,bool isfunction) const;

	void PrintFunction(std::ostream& os, const Class& c);
