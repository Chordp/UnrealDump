...
public:
	...
	Base Cast() const
	{
		return Base((UObject*)object.GetPtr());
	}
...
public:
	...
	struct Info
	{
		...
		static Info Create(PropertyType type, size_t size, bool reference, std::string&& cppType)
		{
			...
			return { type, size, reference, IGenerator::GetInstance()->GetOverrideType(cppType) };
		}
	};
...
bool UEObject::IsA() const
{
	...
	{
		if (super.object.GetPtr() == cmp.object.GetPtr())
		{
			return true;
		}
	}
